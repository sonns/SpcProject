...
//            print_r($menu);exit;
    ...
    protected function getPusherInfo($type,$id){
        ...
        if($type !== ADD_REQUEST_BY_TOP)
        {
            ...
            $param['users'] =  $this->getApprovalList($id,$type);
            $this->addTemplateForPusher($param);
        }
    }
    ...
    private function getApprovalList($request_id,$type ){
        $listUser = null;
        if($type === EDIT_REQUEST_BY_STAFF || $type === EDIT_REQUEST_BY_SUB_MANAGE){
            ...
            if(!$this->isHead && $type === ADD_REQUEST_BY_STAFF  ){
                [$this->Notification->getRecipientList('sub-manager')];
            }else{
                [$this->Notification->getRecipientList('manager')];
            }

        }elseif ($type === ADD_REQUEST_BY_MANAGE || $type === EDIT_REQUEST_BY_MANAGE){
            [$this->Notification->getRecipientList('top')];
        }
        elseif ($type >= APPROVE_REQUEST_BY_SUB_MANAGE && $type <= RETURN_REQUEST_BY_TOP ){
            ...
            $result[] = $requestDetail->user_id;
            if($this->isHead){
                if ($type === APPROVE_REQUEST_BY_MANAGE ){
                    $result[] = $this->Notification->getRecipientList('top');
                }
                if($type === APPROVE_REQUEST_BY_TOP || $type === REJECT_REQUEST_BY_TOP || $type === RETURN_REQUEST_BY_TOP ){
                    $result[] = $this->Notification->getRecipientList('manager');
                }
            }else{
                if ($type === APPROVE_REQUEST_BY_SUB_MANAGE ){
                    $result[] = $this->Notification->getRecipientList('manager');
                }
                if ($type === APPROVE_REQUEST_BY_MANAGE || $type === REJECT_REQUEST_BY_MANAGE || $type === RETURN_REQUEST_BY_MANAGE ){
                    $result[] = $this->Notification->getRecipientList('sub-manager');
                    if($type === APPROVE_REQUEST_BY_MANAGE){
                        $result[] = $this->Notification->getRecipientList('manager');
                    }
                }
